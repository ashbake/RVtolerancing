...
if __name__=='__main__':
    ...
    for itol in np.arange(0,54):#,len(surf_name)):
        if surf_num[itol]<0:
            continue
        val = load_final_data(itol,filename='tol_rv_rate.csv',target_rv_err=0.01)
        if val==0:
            continue
        # shape out: norders, nwavelengths ( take zero index order - already doing just one order to start)
        print(itol)
        X1,Y1,RVerr1,V1,Xshift1,Yshift1 = get_one_shift(surf_num[itol], axis[itol],  val, nom_val[itol], orders, minwls, maxwls,surf_name=surf_name[itol])
        plot_psf_shifts(X1,Y1,Xshift1,Yshift1,RVerr1,surf_name[itol],val)
        #f.write('%s,%s,%s,%s,%s\n' %(surf_name[itol],itol,avg_rate,std_rate,jason_comp))
        plt.savefig('./outputs/surf_%s_RVerror.png'%surf_name[itol])
    #f.close()
